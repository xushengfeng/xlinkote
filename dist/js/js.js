...
document.getElementById("画布").onwheel = (e) => {
    if (e.ctrlKey) {
        ...
        if (e.shiftKey && !e.deltaX) {
            if (fxsd == 0 || fxsd == 2)
                O.style.left = el_offset(O).x - e.deltaY + "px";
        }
        else {
            if (fxsd == 0 || fxsd == 2)
                O.style.left = el_offset(O).x - e.deltaX + "px";
            if (fxsd == 0 || fxsd == 1)
                O.style.top = el_offset(O).y - e.deltaY + "px";
        }
    }
    data_changed();
};
...
document.getElementById("录音").onclick = () => {
    let x = -el_offset(O).x + 8, y = -el_offset(O).y + 8;
    let xel = document.createElement("x-x");
    xel.style.left = x / zoom + "px";
    xel.style.top = y / zoom + "px";
    z.push(xel);
...
class markdown extends HTMLElement {
    ...
    connectedCallback() {
        ...
        text.onclick = text.onkeyup = () => {
            ...
            if (index_i) {
                let el = s.querySelector(`#h > ${index_i[0]}`);
                let x = el_offset2(el, this.h).x, y = el_offset2(el, this.h).y + el.offsetHeight;
...
function line_el(l) {
    ...
    for (let i of l) {
        ...
        if (!el_n[n])
            el_n[n] = {};
        if (i.nesting != -1)
            el_n[n][i.tag] = (el_n[n]?.[i.tag] || 0) + i.nesting;
        el_path[n] = i.tag;
        if (i.tag == "p" && el_path?.[n - 1] == "li")
            continue;
        let t = [];
        for (let e = 0; e <= n; e++) {
    ...
    draw(e) {
        ...
        if (this.points.length != 1) {
            ...
            let so = (i) => {
                ...
                if (this.pen.zoom)
                    w = w / zoom;
                let x0 = this.points[i - 2].x, y0 = this.points[i - 2].y, x1 = this.points[i - 1].x, y1 = this.points[i - 1].y, x2 = this.points?.[i]?.x || x, y2 = this.points?.[i]?.y || y;
        ...
        if (this.points.length == 1 || this.points.length == 2) {
            ...
            let a1 = Math.atan2(this.points[this.points.length - 1].y - this.points[this.points.length - 2].y, this.points[this.points.length - 1].x - this.points[this.points.length - 2].x);
            let a2 = Math.atan2(this.points[this.points.length - 1].y - y, this.points[this.points.length - 1].x - x);
            let s = Math.sqrt((this.points[this.points.length - 1].y - y) ** 2 + (this.points[this.points.length - 1].x - x) ** 2);
            if (s >
                ...
                navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
                    ...
                    mediaRecorder.onstop = () => {
                        ...
                        a.onload = () => {
                            audio.src = a.result;
                            if (!集.assets[this.id])
                                集.assets[this.id] = { url: "", base64: "", sha: "" };
                            集.assets[this.id].base64 = a.result;
                            集.assets[this.id].sha = window.CryptoJS.SHA256(a.result).toString();
                        };
